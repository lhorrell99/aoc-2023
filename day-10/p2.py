...
start_dirs = sorted(
    [d for d in deltas.keys() if get_start_dirs(d, loc, pipes, deltas, map)]
)

start_piece = [
    k for k in pipes.keys() if sorted(list(pipes[k].values())) == start_dirs
].pop()

# Replace start piece
map[loc] = start_piece

print(recurse_find_corners(loc, (start_dirs.pop(), loc), 0, pipes, deltas, map))
